  ...
  async function loop() {
    ...
    if (video.readyState >= 2) {
      ...
      if (W && H) {
        ...
        /* ROI 전처리: 상단 65% → 전체 크기로 확대 전송 (원거리 탐지 향상) */
        if (!roiCanvas) roiCanvas = document.createElement('canvas');
        roiCanvas.width  = W;
        roiCanvas.height = H;
        const rctx = roiCanvas.getContext('2d');
        rctx.drawImage(proc, 0, 0, W, Math.round(H * 0.65), 0, 0, W, H);

        let signals = [];
        try {
          const raw = await runYolo(roiCanvas, W, H);
          /* ROI 좌표 역변환: y축 × 0.65 */
          signals = raw.map(s => {
            const [y1, x1, y2, x2] = s.box;
            return { ...s, box: [y1 * 0.65, x1, y2 * 0.65, x2] };
